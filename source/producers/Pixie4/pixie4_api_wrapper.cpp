...
bool Pixie4Wrapper::boot(const std::vector<std::string> &boot_files)
{
  ...
  {
    ERR("<Pixie4Wrapper> boot_files.size() != 7");
    return false;
  }

  if (!get_sys("NUMBER_MODULES"))
  {
    ERR("<Pixie4Wrapper> No valid module slots.");
    return false;
  }
...
bool Pixie4Wrapper::start_run(uint8_t mod, uint16_t runtype)
{
  ...
  case 0x10:
    return true;
  case -0x11:
    ERR("<Pixie4Wrapper> Start run failed: Invalid Pixie module number");
    return false;
  case -0x12:
    ERR("<Pixie4Wrapper> Start run failed. Try rebooting");
    return false;
  default:
    ERR("<Pixie4Wrapper> Start run failed. Unknown error");
    return false;
  }
}
...
bool Pixie4Wrapper::resume_run(uint8_t mod, uint16_t runtype)
{
  ...
  case 0x20:
    return true;
  case -0x21:
    ERR("<Pixie4Wrapper> Resume run failed: Invalid Pixie module number");
    return false;
  case -0x22:
    ERR("<Pixie4Wrapper> Resume run failed. Try rebooting");
    return false;
  default:
    ERR("<Pixie4Wrapper> Resume run failed. Unknown error");
    return false;
  }
}
...
bool Pixie4Wrapper::stop_run(uint8_t mod, uint16_t runtype)
{
  ...
  case 0x30:
    return true;
  case -0x31:
    ERR("<Pixie4Wrapper> Stop run failed: Invalid Pixie module number");
    return false;
  case -0x32:
    ERR("<Pixie4Wrapper> Stop run failed. Try rebooting");
    return false;
  default:
    ERR("<Pixie4Wrapper> Stop run failed. Unknown error");
    return false;
  }
}
...
bool Pixie4Wrapper::read_EM(uint32_t* data, uint8_t mod)
{
  ...
  switch (retval) {
  case -0x93:
    ERR("<Pixie4Wrapper> Failure to read list mode section of external memory. Reboot recommended.");
    return false;
  case -0x95:
    ERR("<Pixie4Wrapper> Invalid external memory I/O request. Check run type.");
    return false;
  case 0x90:
    return true;
  case 0x0:
  ...
  default:
    ERR("<Pixie4Wrapper> Unexpected error {}", retval); // \todo hex
    return false;
  }
}
...
bool Pixie4Wrapper::read_EM_double_buffered(uint32_t* data, uint8_t mod)
{
  ...
  if  (TstBit(CSR_DATAREADY, (U16)CSR) == 0 )
  {
    ...
    j=1-j;
    WARN("<Pixie4Wrapper> read_EM_double_buffered: module {} csr reports both memory blocks"
         " full (block {} older). Run paused (or finished).", mod, (1-j));
  }
...
{
  DBG("<Pixie4Wrapper> Setting {} to {} for system", setting, val);
  //check bounds?
...
{
  DBG("<Pixie4Wrapper> Getting all system params.");
...
{
  DBG("<Pixie4Wrapper> get ADC (oscilloscope) traces");
