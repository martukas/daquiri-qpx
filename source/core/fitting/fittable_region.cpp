...
double FittableRegion::chi_sq(const Eigen::VectorXd& fit) const
{
  double chi_squared {0.0};
  for (const auto& p : data.data)
    chi_squared += square((p.y - this->eval_at(p.x, fit)) / p.weight_phillips_marlow);
  return chi_squared / degrees_of_freedom();
}
                                       ...
                                       Eigen::VectorXd& gradients) const
{
  ...
  for (const auto& p : data.data)
  {
    ...
    double fit_value = this->eval_grad_at(p.x, fit, channel_gradients);
    chi_squared += square((p.y - fit_value) / p.weight_phillips_marlow);

    double grad_norm = -2.0 * (p.y - fit_value) / square(p.weight_phillips_marlow);
